...
function WrappedTick({ x, y, payload }) {
  ...
  const lines =
    parts.length === 1 && str.length > 12
      ? [str.slice(0, Math.ceil(str.length / 2)), str.slice(Math.ceil(str.length / 2))]
      : parts;

  return (
    <text x={x} y={y + 10} textAnchor="middle" fontSize={11} fill={BRAND.primaryDark}>
      {lines.map((p, i) => (
...
export default function Page() {
  ...
  const base = useMemo(
    () => ({
      totals: { co2e: 14.76, water: 9959.3, land: 62.7 },
      co2Categories: [
      ...
      metrics: { carroKm: 82, banhoHoras: 11.1, areaApto: 1.0 },
    }),
    []
  );
  ...
  const scaled = useMemo(() => {
    ...
    const landData = base.landByMeal.map((d) => ({
      meal: d.meal,
      m2: Math.round(d.m2 * multiplier * 10) / 10,
    }));
  ...
  const fmtKg = (v) =>
    `${Number(v).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg CO₂e`;
        ...
        <main className="flex-1 rounded-2xl bg-[#fff4f8] border border-[#f0e6ef] p-6 lg:p-10 shadow-lg min-h-[640px] w-full">
          <div className="w-full text-[#2f6b46]">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                <h1 className="text-4xl font-extrabold" style={{ color: BRAND.primary }}>Resumo</h1>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              <div>
                <div className="rounded-xl bg-[#f8fff6] p-6 shadow-sm lg:col-span-1" style={{ height: "11rem" }}>
                  <h2 className="text-xl font-semibold mb-3" style={{ color: BRAND.primary }}>Resumo rápido</h2>

                  <div className="space-y-3 text-[#4b6a54]">
                    <div className="flex items-center justify-between" style={{ fontSize: "0.75rem" }}>
                      {fmtKg(scaled.totals.co2e)} de carbono emitido
                    </div>

                    <div className="flex items-center justify-between" style={{ fontSize: "0.75rem" }}>
                      {fmtLit(scaled.totals.water)} de água usada
                    </div>

                    <div className="flex items-center justify-between" style={{ fontSize: "0.75rem" }}>
                      {fmtM2(scaled.totals.land)} de terra usada
                    </div>
                  </div>
                </div>
                <div className="mt-6 space-y-4">
                  ...
                  <MetricCard icon={<MdLandscape size={20} />} title={`${scaled.metrics.areaApto} apt.`} subtitle="Área equivalente (aptos)" bg={BRAND.accent} />
                </div>
              </div>
              ...
              <div className="rounded-xl bg-[#f6fff9] p-6 shadow-sm lg:col-span-3 flex flex-col">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xl font-semibold" style={{ color: BRAND.primary }}>Visualizações</h3>
                  <div className="text-sm text-[#6b6b6b]">Período: <strong>{period}</strong></div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-3 bg-white rounded-md shadow-sm flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                      <strong className="text-sm" style={{ color: BRAND.primary }}>CO₂e por categoria</strong>
                      <span className="text-xs text-[#6b6b6b]">clique em uma barra</span>
                    </div>

                    {/* <-- aqui: wrapper recebeu altura fixa */}
                    <div style={{ height: 230 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={scaled.co2Data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="name" interval={0} tick={<WrappedTick />} height={56} />
                          <YAxis />
                          <Tooltip />
                          <Legend />
                  ...
                  <div className="p-3 bg-white rounded-md shadow-sm flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                      <strong className="text-sm" style={{ color: BRAND.primary }}>Consumo de água (amostra)</strong>
                    ...
                    <div style={{ height: 230 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="day" interval={0} tick={{ fill: BRAND.primaryDark, fontSize: 11 }} />
                          <YAxis />
                          <Tooltip />
                  ...
                  <div className="p-3 bg-white rounded-md shadow-sm flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                      <strong className="text-sm" style={{ color: BRAND.primary }}>Uso de terra por refeição</strong>
                    ...
                    <div style={{ height: 230 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="meal" interval={0} tick={<WrappedTick />} height={56} />
                          <YAxis />
                          <Tooltip />
                    ...
                    <div className="mt-3 text-xs text-[#4b6a54] min-h-[32px]">
                      {selectedLand ? (
                        <span>{selectedLand.meal}: <strong>{selectedLand.m2} m²</strong></span>
                      ) : (
            ...
            <section className="rounded-xl bg-[#fffafc] p-6 mt-6">
              <h2 className="text-2xl font-bold mb-4" style={{ color: BRAND.primary }}>Possíveis substituições</h2>

              <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-6">
                <SubstitutionItem imageSrc="/alimentos/pasta-amendoim.jpeg" meal="Café da manhã" original="Queijo Prato / Presunto" alternative="Pasta de Amendoim" description="A pasta de amendoim (Paçoca) tem uma pegada de carbono 4x menor (133 gCO₂e/100g) que os frios do seu sanduíche, sendo uma excelente troca." />

                <SubstitutionItem imageSrc="/alimentos/arroz-feijao.png" meal="Almoço" original="Bisteca Bovina" alternative="Feijão com Arroz (Vegano)" description="O consumo de Feijão com Arroz (90 gCO₂e/100g) elimina o pico de 10.2 kg CO₂e causado pela carne bovina na sua refeição." />

                <SubstitutionItem imageSrc="/alimentos/file-peixe.jpg" meal="Jantar" original="Hambúrguer Bovina (Cheeseburger)" alternative="Filé de Peixe Grelhado" description="Trocar por Peixe de Água Doce Grelhado (615 gCO₂e/100g) reduz o impacto do seu sanduíche em 38% e elimina a pegada hídrica de água verde/azul." />
      ...
      <div className="w-full flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="text-sm font-semibold" style={{ color: BRAND.primary }}>
            Em seu <span className="font-bold">{meal}</span>, o produto <span className="italic font-bold">{original}</span> pode ser substituído por <span className="font-bold">{alternative}</span>.
          </div>
